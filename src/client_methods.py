...
class Mixin:
    ...
    def account_reset_license_key(self, id_, model):
        if ('X-Avalara-Client' in self.client_header): 
            self.client_header['X-Avalara-Client']=self.client_id.replace("API_VERSION","25.6.2")   
    ...
    ### Security Policies
      ...
      * This API depends on the following active services:*Required* (all): AvaTaxPro, ECMEssentials, ECMPro, ECMPremium, VEMPro, VEMPremium, ECMProComms, ECMPremiumComms.
      ...
      :param companyId [int] The unique identifier of the company whose locations are being requested.
      ...
      :return FetchResult
    """
    def list_location_by_company(self, companyId, include=None):
        if ('X-Avalara-Client' in self.client_header): 
            self.client_header['X-Avalara-Client']=self.client_id.replace("API_VERSION","25.6.2")   
        return requests.get('{}/api/v2/companies/{}/clerk/locations'.format(self.base_url, companyId),
                               auth=self.auth, headers=self.client_header, params=include, 
                               timeout=self.timeout_limit if self.timeout_limit else 1200)
    r"""
    Swagger Name: AvaTaxClient
