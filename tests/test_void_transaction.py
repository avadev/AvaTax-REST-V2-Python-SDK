...
def test_void_without_company_code_raises_errror(single_transaction, auth_client):
    """Test error is raised when called without all params."""
    with pytest.raises(ValueError):
        auth_client.void_transaction(single_transaction, {'code': 'DocVoided'})


def test_void_without_transaction_num_raises_error(single_transaction, auth_client):
    """Test error is raised when called without all params."""
    with pytest.raises(ValueError):
